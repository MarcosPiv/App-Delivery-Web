...
export const getCategories = async (): Promise<Categoria[]> => {
    try {
        const response = await axios.get(`${BASE_URL}/categoria/mostrarTodos`);
...
export const updateCategory = async (id: number, data: Partial<Categoria>): Promise<Categoria> => {
    try {
        const response = await axios.put(`${BASE_URL}/categoria/modificarCategoria/${id}`, data);
        return response.data as Categoria;
    } catch (error) {
...
export const deleteCategory = async (id: number): Promise<void> => {
    try {
        await axios.delete(`${BASE_URL}/categoria/borrarCategoria/${id}`);
    } catch (error) {
